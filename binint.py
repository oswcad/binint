...
# inputs
no_bits = st.sidebar.number_input(
    "Size in bits", min_value=1, max_value=13, value=4, step=1, format="%d"
)

number = st.sidebar.text_input(
    "Decimal number to be converted", placeholder="Range will be checked based on bits"
)

bin_integer = st.sidebar.radio(
    "Desired binary representation", ("Natural binary", "2's complement")
)
...
with tab1:
    ...
    st.write(
        f"The base 2 is raised to the power of position, starting from position 0 at the right, like so:"
    )
    st.dataframe(data=table)
    st.markdown(
        f"Position {headers[-1]} with weight 1 is referred to as **LSB** (*Least Significant Bit*)"
    )
    st.markdown(
        f"Position {headers[0]} with weight {powers[0]} is referred to as **MSB** (*Most Significant Bit*)"
    )
    st.markdown(
        f"In a decimal number such as 2036, each symbol is called a *digit*. In binary we use symbols, **0**, **1** and we call them **bits**"
    )
    st.markdown(
        f"Use tab *Natural binary* to undertand writing binary numbers for whole positive numbers such as 0, 3, 14 etc."
    )
    st.markdown(
        f"Use tab *2's complement* to undertand writing binary numbers for integers; that is positive numbers such as 0, 3, 14 etc. and also negative numbers such as -1, -23, etc."
    )
    ...
    if bin_integer == "Natural binary":
        if number == "":
            ...
            if (value >= lmax) or (value < lmin):
                st.sidebar.warning(
                    f"With {no_bits} bits decimal value must be in range [{lmin}, {lmax-1}] for natural binary"
                )
                st.stop()
            else:
                ...
                if explain_binary:
                    ...
                    st.dataframe(data=table)
                    st.markdown(
                        f"The powers of the binary representation by position for {no_bits} bits is {powers}"
                    )
                    st.markdown(
                        f"Position {headers[-1]} with weight 1 is referred to as **LSB** (*Least Significant Bit*)"
                    )
                    ...
                    bintable = pd.DataFrame(
                        rows, columns=headers, index=["Weights", "Bits"]
                    )
                    ...
                    if more_binary:
                        ...
                        st.write(
                            "we typically give a label to each bit, as A, B, C etc."
                        )
                        frt = "#0" + str(5) + "b"
                        table = [list(format(i, frt)[2:]) for i in range(2**3)]
                        table_df = pd.DataFrame(
                            np.array(table), columns=["A", "B", "C"]
                        )
...
with tab3:
    if bin_integer == "2's complement":
        if number == "":
            ...
            value = int(number)
            l2min, l2max = -(2 ** (no_bits - 1)), (2 ** (no_bits - 1)) - 1
            if (value > l2max) or (value < l2min):
                st.sidebar.warning(
                    f"With {no_bits} bits, decimal value must be in range [{l2min}, {l2max}] for 2's complement"
                )
                st.stop()
            else:
                if value >= 0:
                ...
                if explain_complement:
                    if value >= 0:
                        st.markdown(
                            f"In 2's complement form the *maximum* positive value for *p* bits is:"
                        )
                        st.latex("2^{p-1}-1")
                        st.write(
                    ...
                    if value < 0:
                        st.markdown(
                            f"In 2's complement form the *mimimum* negative value for *p* bits is:"
                        )
                        st.latex("-2^{p-1}")
                        st.write(
                        ...
                        c2n = 2**no_bits - (-value)
                        st.write(
                            f"In summary, c2[{-value}] = {lpow} - {-value} = {2**no_bits} - {-value} = {c2n}"
                        )
                        st.markdown(
                            f"- Then,  the two's complement of {value} is the same bit pattern as the natural binary of {c2n}; that is **{n2_binary}** "
                        )
                        st.write(
                            f"- Note that the MSB of {n2_binary} is 1 in this case"
                        )
                        more_complement = st.checkbox("Simpler please ...")
                        if more_complement:
                            st.write(
                                "Perhaps, simpler to undertand this way. For instance with 3 bits:"
                            )
                            st.write(
                                f"- Build the table of all 8 numbers ({numsuper[3]}) in natural binary"
                            )
                            st.write(
                                f"- Starting from zero, and up to half the table are positive numbers"
                            )
                            st.write(
                                f"- From the last entry of the table, start from -1 and down to the second half are negative numbers"
                            )
                            frt = "#0" + str(5) + "b"
                            table = [list(format(i, frt)[2:]) for i in range(2**3)]
                            table_df = pd.DataFrame(
